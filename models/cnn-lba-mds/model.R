...
setwd('~/lba-cnn')
source('models/cnn-lba-mds/helper_functions.R')
...
opt = list()
opt$nmc = num_samples
opt$file_out = file.path('output',network,'mds',paste0('samples_cat',category,'.RData'))
opt$file_in = file.path('data',network,'mds','dat.RData')
...
data = lapply(unique(dat$subject), 
              function(x) data.frame(subject = dat$subject[dat$subject==x], rt=dat$rt[dat$subject==x]/1000, response = dat$response[dat$subject==x],
                                     dim1 = dat$dim1[dat$subject==x],dim2 = dat$dim2[dat$subject==x],dim3 = dat$dim3[dat$subject==x],dim4 = dat$dim4[dat$subject==x],
                                     dim5 = dat$dim5[dat$subject==x]))
...
theta.names = c(theta.names,paste("w",1:(dims),sep="."))
...
tmp=grep("w",theta.names,value=TRUE)
for (n in 1:length(tmp)) {
     tmp2=tmp[n]
     prior[[tmp2]]=list(mu=c(1,1),sigma=c(1,1))
}
