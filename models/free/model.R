...
source('models/free/helper_functions.R')
...
opt = list()
opt$nmc = num_samples
opt$file_out = file.path('output','free','samples.RData')
opt$file_in = file.path('data','free','dat.RData')
...
dat$cond[dat$viewpoint == 'same' & dat$object == 'same' ] = '1'
dat$cond[dat$viewpoint == 'diff' & dat$object == 'same'] = '2'
dat$cond[dat$viewpoint == 'same' & dat$object == 'diff' ] = '3'
dat$cond[dat$viewpoint == 'diff' & dat$object == 'diff'] = '4'
...
data = lapply(unique(dat$subject), 
              function(x) data.frame(subject = dat$subject[dat$subject==x], rt=dat$rt[dat$subject==x]/1000, acc = dat$acc[dat$subject==x],
                                     cond=dat$cond[dat$subject==x]))
...
doesDriftRateCorrectVary = 1     # Does the mean drift rate for the correct accumulator vary over conditions for this model?
...
if (doesDriftRateCorrectVary) {theta.names=c(theta.names,paste("vc",conds,sep="."))} else {theta.names=c(theta.names,"vc")}
if (doesStDevErrorVary) {theta.names=c(theta.names,paste("svc",conds,sep="."))} else {theta.names=c(theta.names,"svc")}
...
for(i in 1:n.chains){
     ...
     phi[i,tmp,1]=rtnorm(n=length(tmp),mean=.1,sd=.05,0,Inf)
     tmp=grep("vc",phi.names)
     phi[i,tmp,1]=rtnorm(n=length(tmp),mean=4,sd=1,0,Inf)
...
tmp=grep("vc",theta.names,value=TRUE)
for (n in 1:length(tmp)) {
     tmp2=tmp[n]
     prior[[tmp2]]=list(mu=c(3,3),sigma=c(3,3))
}
...
thin_nmc = seq(1,nmc,by=thin_steps)
theta = theta[,,,thin_nmc]
phi = phi[,,thin_nmc]
